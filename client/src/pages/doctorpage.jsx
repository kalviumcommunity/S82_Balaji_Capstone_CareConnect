...
function DoctorsPage() {
  ...
  const fetchDoctors = async () => {
    try {
      const res = await axios.get(`https://s82-balaji-capstone-careconnect-3.onrender.com/api/doctors/specialty/${specialty}`);
      setDoctors(res.data || []);
    } catch (err) {
  ...
  const fetchProfile = async () => {
    ...
    if (token) {
      try {
        const res = await axios.get('https://s82-balaji-capstone-careconnect-3.onrender.com/api/patients/profile', {
          headers: {
  ...
  const handleDelete = async (id) => {
    const token = localStorage.getItem('token');
    try {
      await axios.delete(`https://s82-balaji-capstone-careconnect-3.onrender.com/api/doctors/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
